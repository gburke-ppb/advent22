...
pub fn solution() {
  ...
  let target = 30000000;
  let mut candidates_for_deletion: Vec<u32> = dirs
    .iter()
    .cloned()
    .filter(|n| (unused_space + n) >= target)
    .collect();
...
fn parse_cd(input: &str) -> IResult<&str, Line> {
  tuple((tag("$ cd "), take_while(is_valid_path_char)))(input)
    .map(|(i, (_, o))| (i, Line::Cd(o.to_owned())))
}

// Parse the ls command
fn parse_ls(input: &str) -> IResult<&str, Line> {
  tag("$ ls")(input).map(|(i, _o)| (i, Line::Ls))
}

// Parse the dir entry
fn parse_dir(input: &str) -> IResult<&str, Line> {
  tuple((tag("dir "), alpha1))(input).map(|(i, (_, o))| {
    let d = Dir {
      name: o.to_string(),
      files: Vec::new(),
      dirs: Vec::new(),
...
fn parse_file(input: &str) -> IResult<&str, Line> {
  tuple((decimal, tag(" "), take_while(is_valid_path_char)))(input).map(|(i, (size, _, name))| {
    let f = File {
      name: name.to_string(),
      size,
    };
  ...
  fn test_parse_cd() {
    let table = vec![
      ...
      (
        "$ cd fdshjfsd.txt",
        "",
        Line::Cd("fdshjfsd.txt".to_string()),
      ),
    ];
